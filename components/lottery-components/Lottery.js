...
const Lottery = ({ name, admin, vaultId, tokenString, tContract, endTime }) => {
  ...
  const TooltipContent = () => {
    return (
      ...
      >
        <i>
          Please Note: Each &apos;HACT&apos; balance on a staking page is
          separate to each collection, and that collections stake-to-win
          lottery. Alien Horror Apes HACT cannot be used for Project 333
      ...
      <div className="flex-1">
        ...
        <div className="space-y-5 md:space-y-0 m-5 md:flex md:flex-row items-start justify-center md:space-x-5">
          <div className="stats-container">
            <h1 className="text-4xl text-white font-semibold text-center md:text-5xl">
              {" "}
              The Next Draw{" "}
            </h1>
            <div className="flex justify-between p-2 space-x-2">
              <div className="stats">
                ...
                <p className="text-xl extra font-semibold text-yellow-400">
                  {totalPool} HACT
                </p>
              </div>
              <div className="stats">
                <h2 className="text-sm">You Own</h2>
                <p className="text-xl extra font-semibold text-yellow-400 flex flex-row justify-center">
                  <span id="balance">{0} &nbsp;</span>&nbsp;HACT&nbsp;{" "}
                  <Tooltip
                    placement="leftStart"
          ...
          <div className="stats-container space-y-2">
            <div className="stats-container">
              ...
              <div className="flex items-center space-x-2 text-white bg-[#140f06] border-[#423929] border p-4">
                ...
                <input
                  ...
                  type="number"
                  onChange={(e) => setParams(Number(e.currentTarget.value))}
                />
              </div>
              ...
              {isActive ? (
                <>
                  {allow ? (
                    ...
                    <button
                      ...
                      className="mt-5 w-full bg-gradient-to-br from-[#F5A524] to-gray-600 px-10 py-5 rounded-md text-white shadow-xl disabled:from-gray-600 disabled:text-gray-100 disabled:to-gray-600 disabled:cursor-not-allowed"
                    >
                      Buy {quantity} entries for {totalCost} HACT
                    </button>
                  )}
                </>
              ) : (
                <button
                  disabled={true}
                  className="mt-5 w-full bg-gradient-to-br from-[#F5A524] to-gray-600 px-10 py-5 rounded-md text-[#F5A524] shadow-xl disabled:from-gray-600 disabled:text-gray-100 disabled:to-gray-600 disabled:cursor-not-allowed"
                >
            ...
            <div className="stats text-center">
              <p className="text-lg extra mb-2">
                You have {userEntries} entries in this draw{" "}
              </p>
            </div>
          </div>
        </div>
